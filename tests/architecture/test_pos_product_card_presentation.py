...
def test_concrete_products_alone_resolve_product_card_presentation() -> None:
    ...
    assert product_map.index("</button>\n                      <button type=\"button\" className=\"pos-sale-product-favorite\"") > product_map.index('className="pos-sale-product-card-select"')
...
def test_fallback_and_photo_css_preserve_required_dimensions() -> None:
    ...
    fallback_label = r"\.pos-sale-product-card--without-image[^\{]*\.pos-sale-product-card-select\s*>\s*span\s*\{[^}]*"
