...
const PCO007Reports = () => {
  ...
  const [tab, setTab] = useState<Tab>(ingredientAllowed ? 'ingredient' : 'expenses');
  ...
  const [cursors, setCursors] = useState<Record<Tab, string | null>>({ ingredient: null, expenses: null });
  ...
  return <main className="sales-monitor-page">
    <header className="sales-monitor-header"><div><h1>Reportes históricos</h1><p>Consulta autoritativa; cantidades, impuestos e importes se calculan en el backend.</p></div><span>Zona horaria: {timeZone}</span></header>
