...
interface IngredientVariation extends VariationNote { variation_id: string; effect_type: 'add' | 'remove'; inventory_item_name?: string; }
...
export default function BranchAdminVariations() {
  ...
  useEffect(() => { void load(); }, []);
  const visible = useMemo(() => { const source = tab === 'notes' ? notes : ingredients; const q = search.trim().toLowerCase(); return !q ? source : source.filter((note) => `${note.product_name} ${note.name} ${(note as IngredientVariation).inventory_item_name || ''}`.toLowerCase().includes(q)); }, [notes, ingredients, search, tab]);
  ...
  return <BranchAdminPage title="Variaciones y cambios" description={`Disponibilidad local por acción${branchName ? ` para ${branchName}` : ''}.`} icon={MessageSquareText}>
    ...
    {loading ? <p>Cargando variaciones…</p> : visible.length === 0 ? <p style={{ color: '#64748b' }}>No hay cambios que coincidan.</p> : <div style={{ display: 'grid', gap: 8 }}>{visible.map((note) => <div key={note.option_id} style={{ padding: 12, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><div style={{ flex: 1 }}><strong>{note.name}</strong><div style={{ fontSize: 12, color: '#64748b' }}>{tab === 'ingredients' && `${(note as IngredientVariation).inventory_item_name || 'Insumo'} · ${(note as IngredientVariation).effect_type === 'add' ? 'Con' : 'Sin'} · `}{note.product_name} · Central: {note.central_status}</div></div><span style={{ color: note.effective_enabled ? '#047857' : '#b91c1c', fontSize: 13 }}>{note.effective_enabled ? 'Disponible' : 'No disponible'}</span>{canManage && <div style={{ display: 'flex', gap: 6 }}><button disabled={acting === note.option_id} onClick={() => void setAvailability(note.option_id, 'available')}>Disponible</button><button disabled={acting === note.option_id} onClick={() => void setAvailability(note.option_id, 'unavailable')}>No disponible</button><button disabled={acting === note.option_id} onClick={() => void setAvailability(note.option_id, 'inherit')}>Heredar</button></div>}</div>)}</div>}
  </BranchAdminPage>;
}
