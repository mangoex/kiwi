...
export default function BranchAdminVariations() {
  ...
  const load = async () => {
    ...
    try { setNotes(await fetchApi<VariationNote[]>('/branch-administration/catalog/variation-notes')); }
    catch (reason) { setError(reason instanceof ApiError ? reason.message : 'No fue posible cargar las variaciones.'); }
    finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  const visible = useMemo(() => { const q = search.trim().toLowerCase(); return !q ? notes : notes.filter((note) => `${note.product_name} ${note.name}`.toLowerCase().includes(q)); }, [notes, search]);
  const setAvailability = async (optionId: string, action: 'available' | 'unavailable' | 'inherit') => {
    setActing(optionId); setError('');
    try { await fetchApi(`/branch-administration/catalog/variation-notes/${encodeURIComponent(optionId)}`, { method: 'PUT', body: JSON.stringify({ action }) }); await load(); }
    catch (reason) { setError(reason instanceof ApiError ? reason.message : 'No fue posible actualizar la disponibilidad.'); }
    finally { setActing(null); }
  };
  return <BranchAdminPage title="Variaciones y cambios" description={`Disponibilidad local de notas preestablecidas${branchName ? ` para ${branchName}` : ''}.`} icon={MessageSquareText}>
