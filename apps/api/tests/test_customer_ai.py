...
def sample_crm_data(test_db: Session) -> dict[str, str]:
    ...
    test_db.execute(
        models.product_categories.insert().values(
            id=CATEGORY_ID, organization_id=ORGANIZATION_ID, name="Bebidas", display_order=1, status="active", created_at=now, updated_at=now
        )
    )
    test_db.execute(
        models.products.insert().values(
            id=PRODUCT_A_ID, organization_id=ORGANIZATION_ID, category_id=CATEGORY_ID, name="Maccha Pinku", sku="PROD-MP", station="bar", status="active", created_at=now, updated_at=now
        )
    )
    test_db.execute(
        models.products.insert().values(
            id=PRODUCT_B_ID, organization_id=ORGANIZATION_ID, category_id=CATEGORY_ID, name="Sando Kyoto", sku="PROD-SK", station="kitchen", status="active", created_at=now, updated_at=now
        )
    )
    ...
    test_db.execute(
        models.price_versions.insert().values(
            id="018f6f73-2d0a-74f0-8f1c-000000000095", organization_id=ORGANIZATION_ID, product_id=PRODUCT_A_ID, price_cents=8500, valid_from=now, created_at=now
        )
    )
    test_db.execute(
        models.price_versions.insert().values(
            id="018f6f73-2d0a-74f0-8f1c-000000000096", organization_id=ORGANIZATION_ID, product_id=PRODUCT_B_ID, price_cents=14000, valid_from=now, created_at=now
        )
    )
    ...
    test_db.execute(
        models.customers.insert().values(
            id=CUSTOMER_ID, organization_id=ORGANIZATION_ID, origin_branch_id=BRANCH_ID, name="Valeria Silva", email="valeria@example.com", status="active", created_at=now, updated_at=now
        )
    )
    test_db.execute(
...
def test_get_crm_segments_and_churn_risk(
    test_db: Session, sample_crm_data: dict[str, str]
) -> None:
    crm_summary = get_crm_segments_and_churn_risk(
        test_db, branch_id=sample_crm_data["branch_id"]
    )
...
def test_customer_ai_endpoints(
